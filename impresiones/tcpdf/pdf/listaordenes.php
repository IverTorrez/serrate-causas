...
$_SESSION["imgindex"]=$linkimg;


/*------------OBTENEMOS LA CANTIDAD DE ORDENES Y EL NUMERO DE PAGINA QUE TOCA IMPRIMIR*/
  ...
  $numerodePaginas=($cantidadOrdenes/20); //obtenemos el numero de hojas que tnedra el informe
  $totalPaginasEntero= ceil( $numerodePaginas );//redondea hacia adelante
  $numpagina=$_GET['page'];//numero de pagina actual

  
  $indexfinal=$numpagina*20;//multiplicado por la cantidad de ordenes que se mostrara de una pagina(es el numero de orden final a mostrar)
  $indexinicio=$indexfinal-19;//desde que numero se mostrara la orden
  
  if ($numpagina==$totalPaginasEntero)
  {
    $paginasTotales=$totalPaginasEntero-1;
  }
  else
  {
    $paginasTotales=$numpagina-1;
  }
 // $paginasTotales=4;

  $_SESSION["numpag"]=$paginasTotales;
class PDF extends TCPDF
{
   ...
   function Header()
   {
      if ($_SESSION["numpag"]==0)
     {
      ...
      $this->Image($_SESSION["imgindex"],30,3, 70, 15, '', '', '', false, 300, '', false, false, 0, false,false,false);
     //$this->SetFont('','B',12);
   ...
   function Footer()
   {
       $paginasTotales=$_SESSION["numpag"];
     // Position at 15 mm from bottom
        ...
        // Custom footer HTML
        $pagg=$this->PageNo();
        $pag2=$pagg+$paginasTotales;
        $this->html = '<hr><br><span>*Se <?php echo $pagg; ?>  cotiza solo lo detallado, cualquier modificación sera sujeta a un nuevo presupuesto.</span><br><span>*Esta cotización se acepta en conjunto con hoja condiciones comerciales y especificaciones técnicas que deben ser aceptadas y firmada por el cliente.</span>';
        $this->Cell(0,10,'Pagina '.($pag2),0,0,'C');
        //$this->writeHTML($pag2, true, false, true, false, '');  
...
//	$this->Cell(0,10,'Pagina '.($this->PageNo()+$paginasTotales),0,0,'C');
...
$pdf->SetAutoPageBreak(true,10);
 ...
 //$pdf->Cell(0,160,'Pagina '.($pdf->PageNo()+$paginasTotales),0,0,'C');
/*===============================================================
...
}/*fin del if que pregunta si es la pagina uno para mostrar titulo*/
/***************************************LISTADO DE ORDENES********************************/
else/*por falso, imprime el salto de linea para que muestre las ordenes*/
{$pdf->Ln(25);}
...
$totalegreso=0;
   ...
   $resul=$objorden->listarordenesdeunacausaPDF_1($codigonuevo);
 ...
 if ($fil['cont']>=$indexinicio and $fil['cont']<=$indexfinal) 
    {
...
$pdf->writeHTML($bloqueDatosOrdenes,false,false,false,false,'');
    }/*fin del if que pregunta si las ordenes estan dentro del index inicio y index final */


  }/*FIN DEL WHILE QUE RRECORE LAS ORDENES*/

/***************************TABLA TOTAL EGRESOS DE LAS ORDENES*************/
if ($numpagina==$totalPaginasEntero) {/*preguntamos si la pagina actual es la ultima*/
$pdf->SetMargins(34, 30 ,2.5);
...
$pdf->writeHTML($bloqueTotalesCostosOrdenes,false,false,false,false,'');
  ...
  $pdf->Ln(10);
  }/*fin del if que pregunta si la pagina actual es la ultima pagina, para mostrar el total final*/
