...
<html>
<head>
    <title>Ordenes Vencidas Graves</title>
...
<tbody>
 ...
 $concat=$fechoyal.' '.$horita;
   ...
   $resul=$objorden->mostrarVencidasGravesDeCausaCont($codigonuevo);
        ...
        //SE ENCRIPTA EL CODIGO DE LA ORDEN PARA ENVIARLO POR LA URL //
            $mascara=$fil->Codorden*10987654321;
             $encriptado=base64_encode($mascara);
              ...
              $objepresu=new Presupuesto();
              $resulpre=$objepresu->mostrarUnpresupuestoEntregadoDeorden($fil->Codorden);
              $filpre=mysqli_fetch_object($resulpre);
              echo "<td>$filpre->monto_presupuesto</td>";

              $objdesc=new DescargaProcurador();
              $resuldesc=$objdesc->mostrardescargaorden($fil->Codorden);
              $fildesc=mysqli_fetch_object($resuldesc);
              echo "<td>$fildesc->gastos</td>";
              echo "<td>$fildesc->saldo</td>";

              echo "<td>$fil->procuradorasig</td>";

              echo "<td>$fil->fecha_giro</td>";

              echo "<td>$filpre->fecha_presupuesto</td>";
              echo "<td>$filpre->fecha_entrega</td>";
