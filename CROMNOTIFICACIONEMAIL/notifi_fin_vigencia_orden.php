...
include_once('../model/clsplanilla_notificacion.php');
	...
	$resulorden=$objorden->listarDatosDeOrdenesParaNotificarPlazoFinalizo();
	while ($filorden=mysqli_fetch_object($resulorden)) 
	{
		/*$senior = stripslashes('Se簽or'); /*se pasa los datos para que se interprete*/
      /*  $senior = iconv('UTF-8', 'windows-1252', $senior);*/
        
       /* $numero = stripslashes('n繳mero'); /*se pasa los datos para que se interprete*/
        /*$numero = iconv('UTF-8', 'windows-1252', $numero);*/
        
       // $termino = stripslashes('Termin車'); /*se pasa los datos para que se interprete*/
        /*$termino = iconv('UTF-8', 'windows-1252', $termino);*/
		...
		$objordenmarcar->set_notificadoemail('Si-Si');
...
/*-------------FIN DE ENVIO DE NOTIFICACION POR TELEGRAM--------------------------**/
     ...
     enviarNotPuschFinPlazoOrdenOrden($idcausa,$tokenproc,$idorden);
  }/*fin del while que recorre todas las ordenes con plazo finalizado*/
...
// $tokens[] = $fila->token;
  ...
  /*   $msg = array
      (
          ...
          'body'     => 'Terminó el plazo para realizar esta orden.',
...
function enviarNotPuschFinPlazoOrdenOrden($idcausa,$tokenproc,$idorden)
{     /*Seleccionamos la planilla 19*/
      $objplanilla= new Planillas_envio_notificacion();
      $resulplanilla=$objplanilla->mostrarUnaPlanillas_envioNotif_activa(19);
