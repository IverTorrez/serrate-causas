...
<html>
<head>
    <title>Lista de Ordenes Vencidas Graves</title>
...
<h3 style="color: #000000;font-size: 25px;text-align: center;">LISTADO DE ORDENES  VENCIDAS GRAVES</h3>    
    <br>
...
ini_set('date.timezone','America/La_Paz');
 ...
 $concat=$fechoyal.' '.$horita;
   ...
   $resul=$objorden->mostrarVencidasGravesDeCausa($codigonuevo);
        ...
        //SE ENCRIPTA EL CODIGO DE LA ORDEN PARA ENVIARLO POR LA URL //
            $mascara=$fil->Codorden*10987654321;
             $encriptado=base64_encode($mascara);
              ...
              $resulpres=$objpresu->mostrarpresupuesto($fil->Codorden);
              $filpresu=mysqli_fetch_object($resulpres);
              echo "<td>$filpresu->fecha_presupuesto</td>";

              echo "<td>$fil->fecha_recepcion</td>";

              echo "<td>$filpresu->fecha_entrega</td>";

              $objdesc=new DescargaProcurador();
              $resuldesc=$objdesc->mostrardescargaorden($fil->Codorden);
              $fildesc=mysqli_fetch_object($resuldesc);
              echo "<td>$fildesc->fecha_descarga</td>";

              echo "<td>$fil->Inicio</td>";
              echo "<td>$fil->Fin</td>";

              echo "<td>$fil->pronuncabogado</td>";/*ESTOS DATOS NUNCA APARECERAN EN ESTA LISTA... ESTAN VACIOS*/
              echo "<td>$fil->pronuncont</td>";/*ESTOS DATOS NUNCA APARECERAN EN ESTA LISTA... ESTAN VACIOS*/
              echo "<td>$fil->fechacierress</td>";/*ESTOS DATOS NUNCA APARECERAN EN ESTA LISTA... ESTAN VACIOS*/
             ...
             switch ($fil->Condicion) {
              ...
              echo "<td>$fil->Costojudiventa</td>";  //dato no hay aun, este dato depende de si es suficiente la orden
              echo "<td>$fil->Costocompraproc</td>";  //dato no hay aun, este dato depende de si es suficiente la orden
               echo "<td>$fil->Costoventaproc</td>";  //dato no hay aun, este dato depende de si es suficiente la orden
               echo "<td>$fil->totalclien</td>";  //dato no hay aun, este dato depende de si es suficiente la orden

              echo "<td>$fil->procuradorasig</td>";

               echo "<td>$fil->calificacion</td>";  //dato no hay aun, este dato depende de si es suficiente la orden
